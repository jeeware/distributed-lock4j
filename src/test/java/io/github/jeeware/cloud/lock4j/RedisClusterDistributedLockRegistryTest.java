...
class RedisClusterDistributedLockRegistryTest extends DistributedLockRegistryTest {
    ...
    static void redisNodesPorts(DynamicPropertyRegistry registry) {
        registry.add("master1.port", SingletonSupplier.of(SocketUtils::findAvailableTcpPort));
        registry.add("replica1.port", SingletonSupplier.of(SocketUtils::findAvailableTcpPort));
        registry.add("master2.port", SingletonSupplier.of(SocketUtils::findAvailableTcpPort));
        registry.add("replica2.port", SingletonSupplier.of(SocketUtils::findAvailableTcpPort));
    }
    ...
    static class Config {
        ...
        RedisCluster redisCluster(@Value("${master1.port}") int m1Port, @Value("${replica1.port}") int r1Port,
                @Value("${master2.port}") int m2Port, @Value("${replica2.port}") int r2Port) throws IOException {
            return RedisCluster.newRedisCluster()
                    .sentinelCount(0)
